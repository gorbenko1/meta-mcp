...
class MetaAdsClient {
  ...
  async getAdAccounts(): Promise<any[]> {
    ...
    if (response.success) {
      ...
      response.accounts.forEach((account: any, index: number) => {
        ...
        console.log(`   Currency: ${account.currency}, Status: ${account.account_status}`);
      });
  ...
  async getCampaigns(accountId: string): Promise<any[]> {
    ...
    if (response.campaigns) {
      ...
      response.campaigns.forEach((campaign: any, index: number) => {
        ...
        console.log(`   Objective: ${campaign.objective}, Status: ${campaign.status}`);
        if (campaign.daily_budget) {
          console.log(`   Daily Budget: $${(parseInt(campaign.daily_budget) / 100).toFixed(2)}`);
        }
      });
  ...
  async getCampaignPerformance(campaignId: string): Promise<void> {
    ...
    if (response.campaign && response.performance) {
      ...
      console.log(`  Impressions: ${response.performance.total_impressions?.toLocaleString() || 'N/A'}`);
      console.log(`  Clicks: ${response.performance.total_clicks?.toLocaleString() || 'N/A'}`);
      console.log(`  Spend: $${response.performance.total_spend?.toFixed(2) || 'N/A'}`);
      console.log(`  CTR: ${response.performance.average_ctr?.toFixed(2) || 'N/A'}%`);
      console.log(`  CPC: $${response.performance.average_cpc?.toFixed(2) || 'N/A'}`);
    } else {
  ...
  async createTestCampaign(accountId: string): Promise<string | null> {
    ...
    const campaignName = `Test Campaign - ${new Date().toISOString().split('T')[0]}`;
  ...
  async getAudiences(accountId: string): Promise<void> {
    ...
    if (response.audiences) {
      ...
      response.audiences.forEach((audience: any, index: number) => {
        ...
        console.log(`   Type: ${audience.type}, Size: ${audience.approximate_count?.toLocaleString() || 'N/A'}`);
      });
    } else {
