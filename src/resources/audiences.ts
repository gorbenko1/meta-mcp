...
export function registerAudienceResources(server: McpServer, metaClient: MetaApiClient) {
    ...
    async (uri, { account_id }) => {
      try {
        const result = await metaClient.getCustomAudiences(account_id as string, {
          limit: 100,
          fields: ['id', 'name', 'description', 'subtype', 'approximate_count', 'data_source', 'retention_days', 'creation_time', 'operation_status']
        });
        ...
        const healthMetrics = {
          ...
          processing_audiences: audiences.filter(a => a.operation_status?.code !== 200 && a.operation_status?.code !== 400).length,
          failed_audiences: audiences.filter(a => a.operation_status?.code === 400).length,
          average_size: audiences.length > 0 ? 
            audiences.reduce((sum, a) => sum + (a.approximate_count || 0), 0) / audiences.length : 0
        };
        ...
        const overview = {
          ...
          recent_audiences: audiences
            ...
            .map(audience => ({
              ...
              approximate_count: audience.approximate_count,
              creation_time: audience.creation_time,
              days_since_created: Math.floor((Date.now() - new Date(audience.creation_time).getTime()) / (1000 * 60 * 60 * 24))
            })),
  ...
  server.resource(
    ...
    new ResourceTemplate('meta://audience-performance/{account_id}', { list: undefined }),
    async (uri, { account_id }) => {
      try {
        ...
        const audiences = await metaClient.getCustomAudiences(account_id as string, {
          limit: 50,
          fields: ['id', 'name', 'subtype', 'approximate_count', 'creation_time']
        });
        ...
        const audiencePerformance = {
          ...
          audience_metrics: audiences.data.map(audience => ({
            ...
            age_days: Math.floor((Date.now() - new Date(audience.creation_time).getTime()) / (1000 * 60 * 60 * 24)),
          ...
          size_distribution: {
            ...
            medium: audiences.data.filter(a => (a.approximate_count || 0) >= 10000 && (a.approximate_count || 0) < 100000).length,
            large: audiences.data.filter(a => (a.approximate_count || 0) >= 100000 && (a.approximate_count || 0) < 1000000).length,
  ...
  server.resource(
    'targeting-insights',
    new ResourceTemplate('meta://targeting-insights/{account_id}', { list: undefined }),
    async (uri, { account_id }) => {
      try {
  ...
  server.resource(
    'audience-health',
    new ResourceTemplate('meta://audience-health/{account_id}', { list: undefined }),
    async (uri, { account_id }) => {
      try {
        const audiences = await metaClient.getCustomAudiences(account_id as string, {
          limit: 100,
          fields: ['id', 'name', 'subtype', 'approximate_count', 'creation_time', 'operation_status', 'retention_days']
        });
        ...
        const healthReport = {
          ...
          audience_health_details: audiences.data.map(audience => {
            const ageDays = Math.floor((now - new Date(audience.creation_time).getTime()) / (1000 * 60 * 60 * 24));
            ...
            if (healthStatus === 'healthy') healthReport.health_summary.healthy_audiences++;
            else if (healthStatus === 'warning') healthReport.health_summary.warning_audiences++;
...
function generateAudienceRecommendations(subtype: string, size: number): string[] {
  const recommendations: string[] = [];
